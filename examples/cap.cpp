...
int main(void)
{
  ...
  for (;;)
  {
    ...
    if (c1.isTouched())
    {
      int16_t pos = c1.getPosition();  // getPosition() returns a position value from 0 to 100 across the sensor
      int16_t mag = c1.getMagnitude(); // getMagnitude() returns a relative 'pressure' value of about 50 to 500
      for (int i = 0; i < 100; i++)
      {
